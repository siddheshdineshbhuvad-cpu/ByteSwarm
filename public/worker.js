...
async function computeFibonacci(taskId, params) {
  ...
  self.postMessage({
    ...
    currentStep: 0,
    totalSteps: n,
    intermediate: { status: 'Calculating Fibonacci sequence...' }
  });
  ...
  for (let i = 2; i <= n; i++) {
    ...
    if (i % CHUNK_STEP === 0 || i === n) {
      ...
      self.postMessage({
        ...
        currentStep: i,
        totalSteps: n,
        intermediate: { status: `Calculated Fib(${i.toLocaleString()})` }
      });
      await yieldToEventLoop();
    }
  }
